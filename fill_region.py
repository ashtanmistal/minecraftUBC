"""
This function takes in a starting and end coordinate, and finds void regions within the selection. It then fills in the
...
def main():
    ...
    # iterate through each chunk and check for void blocks

    # if the user specified the chunks in opposite order, swap them
    if cx > cx2:
        cx, cx2 = cx2, cx
    if cz > cz2:
        cz, cz2 = cz2, cz
    ...
    for chunk_x in tqdm(range(cx, cx2 + 1)):
        for chunk_z in range(cz, cz2 + 1):
            ...
            # iterate through each block in the chunk. We only need to check y level of min_height because we're only
            # looking for void blocks in the bottom of the world
            # find the block id of air
            ...
            # iterate through each block in the chunk, and if it's air, call hole_filler.py to fill in the void
            # we only need to check y = min_height. No other blocks.
            for x in range(16):
                for z in range(16):
